...
genotypeFactor.card = [0.5 * (numAlleles*(numAlleles+1))];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

for i=1:prod(genotypeFactor.card)
    x = genotypesToAlleles(i, 1);
    y = genotypesToAlleles(i, 2);
    if i==1 || i==prod(genotypeFactor.card)
        genotypeFactor.val(i) = alleleFreqs(x) * alleleFreqs(y);
    else
        genotypeFactor.val(i) = 2 * alleleFreqs(x) * alleleFreqs(y);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  