...
for i=1:length(pedigree.names)
    parVar = pedigree.parents(i, :);
    if parVar(1) * parVar(2) == 0
        factorList(i) = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
        factorList(length(pedigree.names) + i) = phenotypeGivenGenotypeFactor(alphaList, i, length(pedigree.names)+i);
    else
        factorList(i) = genotypeGivenParentsGenotypesFactor(numAlleles, i, parVar(1), parVar(2));
        factorList(length(pedigree.names) + i) = phenotypeGivenGenotypeFactor(alphaList, i, length(pedigree.names)+i);
    end
end
