...
genotypeFactor.card = repmat(0.5 * numAlleles * (numAlleles+1), 1, 3);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

A = IndexToAssignment(1:prod(genotypeFactor.card), genotypeFactor.card);
IndxP1 = genotypesToAlleles(A(:,2), :);
IndxP2 = genotypesToAlleles(A(:,3), :);
IndxCh = genotypesToAlleles(A(:,1), :);

for i=1:length(genotypeFactor.val)
    for x = 1:2
        for y = 1:2
            parents_cross = sort([IndxP1(i,x) IndxP2(i,y)]);
            if(prod(parents_cross == IndxCh(i,:)))
